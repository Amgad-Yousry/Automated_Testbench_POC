...
int main(void)
{
   ...
   // u16PreTriggerDelay = 2000;
    ...
    u16PreTriggerDelay = 466U;
    int32_t u16PreTriggerDelay2 = u16PreTriggerDelay +1U;


	PDB_DRV_Init(PDB_INSTANCE, &pdly1_InitConfig0);
	PDB_DRV_ConfigAdcPreTrigger(PDB_INSTANCE, ADC_CHN, &pdly1_AdcTrigInitConfig0);
	PDB_DRV_SetTimerModulusValue(PDB_INSTANCE, (uint32_t) u16PreTriggerDelay2);
	PDB_DRV_SetAdcPreTriggerDelayValue(PDB_INSTANCE, ADC_CHN, 0U, (uint32_t) u16PreTriggerDelay);
	/* Set PDB1 interrupt delay value */
	// PDB_DRV_SetValueForTimerInterrupt(PDB_INSTANCE,u16PreTriggerDelay);
